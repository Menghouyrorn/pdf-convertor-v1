        ...
        <div className="grid gap-y-4 justify-center">
          ...
          {DATAPAGE.map((item, index) => (
            ...
            >
              <p
                key={index}
                className="w-[60%] max-md:w-[98%] text-center m-auto font-semibold"
              >
                {currentLang ? item.title : item.titleEn}
              </p>
            </motion.div>
          ))}
        </div>
          ...
          {CARD_DATA.map((item, index) => {
            return (
              <CardProduct
                key={index}
                {...item}
                lable={currentLang ? item.labelkh : item.lable}
                des={currentLang ? item.deskh : item.des}
                image={`/${currentLang ? "kh" : "en"}${item.image}`}
              />
            );
          })}
        </div>
